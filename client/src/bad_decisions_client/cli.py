...
def _get_json(url: str, *, timeout: float) -> Any:
    request = Request(url, headers={"Accept": "application/json", "User-Agent": f"bad-decisions-client/{__version__}"})
    try:
...
def run(argv: Sequence[str] | None = None) -> int:
    ...
    if values and values[0] == "health":
        values[0] = "--health"
    elif values and values[0] == "deal":
        values.pop(0)
    ...
    try:
        ...
        print(f"regret: {exc}", file=sys.stderr)
        return 1
