...
### Portable `.carddeck` archives

Share one pack as a `.carddeck` ZIP archive. Version 1 contains exactly
...
The complete interoperable format is specified in [CardDeck 1](CARDDECK.md), with
a machine-readable [manifest schema](schemas/carddeck-manifest-v1.schema.json).

```bash
bad-decisions pack export maha ./maha.carddeck
bad-decisions pack validate ./maha.carddeck
bad-decisions pack init-registry /absolute/pack/registry
bad-decisions pack import ./maha.carddeck /absolute/pack/registry
```
